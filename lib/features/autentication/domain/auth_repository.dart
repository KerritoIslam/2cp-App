...
class AuthRepository {
  ...
  AuthRepository( {required this.localSecureStorage,required this.restAuthRemote});
  ...
  try {
    ...
    return response.fold(
      ...
      (response) async {
        ...
        final tokenResult = await _saveTokens(response.tokens.accessToken, response.tokens.refreshToken);
        return tokenResult.fold(
          (failure) => left(failure), // Return failure if saving tokens fails
          (_) => right(userModelToEntity(response.user)), // Return the user entity on success
        );
      },
    );
  } on Failure catch (e) {
...
Future<Either<Failure, Unit>> _saveTokens(String accessToken, String refreshToken) async {
  try {
    await localSecureStorage.setTokens(accessToken, refreshToken);
    return right(unit); // Return success if tokens are saved
  } on Exception catch (e) {
    return left(Failure('Error saving tokens: $e')); // Return failure if token saving fails
  }
}
  Future<Either<Failure, User>> register(
    ...
    try {
      ...
      return response.fold(
          (failure) => left(failure), (res)async {

         final tokensReponse=await _saveTokens(res.tokens.accessToken, res.tokens.refreshToken);
          return tokensReponse.fold((l) => left(l), (_) =>  right(userModelToEntity(res.user)) );
         }
      );
    } on Failure catch (e) {
