...
class RemoteDataSource {
  ...
  List<Map<String, dynamic>> stupidParser(Map<String, dynamic> json) {
    ...
    final List<Map<String, dynamic>> posts =
        List<Map<String, dynamic>>.from(data['post']);
    final List<Map<String, dynamic>> applications =
        List<Map<String, dynamic>>.from(data['application']);
    ...
    return applications.asMap().entries.map((entry) {
      ...
      final Map<String, dynamic>? matchingPost =
          index < posts.length ? posts[index] : null;
  ...
  Future<Either<Failure, List<ApplicationModel>>>
    ...
    try {
      ...
      if (res.statusCode == 200) {
        final List<ApplicationModel> map =
  ...
  Future<Either<Failure, Unit>> submitApplication(
      ApplicationModel application, File? attachement, int? teamId) async {
    // some code to get data from remote
    try {
      ...
      if (attachement != null) {
        ...
        body.files.add(MapEntry('attechment',
            MultipartFile.fromFileSync(attachement.path, filename: fileName)));
      }
      late final res;
      if (teamId != null) {
        res = await dio.post('app/application/${application.post.id}/',
            queryParameters: {'team': teamId}, data: body);
      } else {
        res = await dio.post('app/application/${application.post.id}/',
            data: body);
      }
