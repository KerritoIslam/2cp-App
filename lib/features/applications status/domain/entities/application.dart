...
class Application extends Equatable {
 ...
 final  String proposal;
  ...
  const Application( {required this.id, required this.proposal,required this.status,required this.opportunity,});
  @override
  List<Object?> get props => [];

  factory Application.fromModel(ApplicationModel model){
    return Application(id: model.id, proposal: model.proposal, status: model.status,  opportunity: model.post.toEntity());
  }
  ApplicationModel toModel(){
 return ApplicationModel(id: id, proposal: proposal, post: opportunity.toModel(),  sumbittedAt:DateTime.now(), type:"NONE") ;
  }
}
